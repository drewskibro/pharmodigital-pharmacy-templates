...
function switchBookingAge(button) {
  document.querySelectorAll('.malaria-toggle-btn').forEach(function (btn) {
    btn.classList.remove('active');
  });
  button.classList.add('active');

  var iframe = document.getElementById('malaria-acuity-iframe');
...
function toggleFAQ(button) {
  const item = button.closest('.malaria-faq-item');
  const isActive = item.classList.contains('travel-faq-active');

  // Close all FAQs
  document.querySelectorAll('.malaria-faq-item').forEach(faq => {
