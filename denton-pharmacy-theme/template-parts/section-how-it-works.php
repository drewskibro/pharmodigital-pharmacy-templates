...
$badge_text  = dp_field( 'hiw_badge_text', 'Simple & Fast' );
$title       = dp_field( 'hiw_title', 'How It Works' );
$description = dp_field( 'hiw_description', 'Getting started with Denton Pharmacy is simple. From consultation to delivery, we\'ve made healthcare accessible.' );
$cta_text    = dp_field( 'hiw_cta_text', 'Start Your Journey Now' );
$cta_url     = dp_field( 'hiw_cta_url', dp_booking_url() );
...
if ( empty( $steps ) ) {
    for ( $i = 1; $i <= 3; $i++ ) {
        $icon  = dp_field( 'hiw_step_' . $i . '_icon', $default_steps[ $i - 1 ]['step_icon'] );
        $stitle = dp_field( 'hiw_step_' . $i . '_title', $default_steps[ $i - 1 ]['step_title'] );
        $sdesc  = dp_field( 'hiw_step_' . $i . '_desc', $default_steps[ $i - 1 ]['step_description'] );
        $stime  = dp_field( 'hiw_step_' . $i . '_time', $default_steps[ $i - 1 ]['step_time'] );
    ...
    <div class="section-container">
        ...
        <div class="how-it-works-steps">
            ...
            <?php foreach ( $steps as $index => $step ) :
                ...
                $step_icon_class = dp_fa_class( $step['step_icon'] );
            ?>
